    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip" style={{ backgroundColor: 'white', padding: '10px', border: '1px solid #ccc' }}>
          ...
          {event && (
            <>
              <p>{`Event: ${event['Event Description']}`}</p>
              <p>{`Type: ${event.Type}`}</p>
            </>
          )}
        </div>
    ...
    <div className="App">
      ...
      <ResponsiveContainer width="95%" height={500}>
        <LineChart data={priceData}>
          ...
          {changepointDate && (
            <ReferenceLine x={changepointDate} stroke="red" strokeDasharray="5 5" label="Change Point" />
          )}
          {eventData.map((event, index) => (
            <ReferenceLine 
              ...
              stroke="#82ca9d" 
              strokeDasharray="5 5" 
              label={{ value: event['Event Description'], position: 'top', angle: -90, dx: -10 }} 
            />
          ))}
        </LineChart>
      </ResponsiveContainer>
